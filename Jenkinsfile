...
pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE = "carlosdelgadillo/division"
        DOCKER_TAG = "latest"
        DOCKERHUB_CREDENTIALS_ID = "docker-hub-credentials"
        DOCKERHUB_REPO = "carlosdelgadillo/division"
    ...
    stages {
            ...
            steps {
                script {
                    ...
                    if [ -n "$CONTAINER_ID" ]; then
                        echo "Deteniendo el contenedor que usa el puerto 8006..."
                        docker stop $CONTAINER_ID
                        docker rm $CONTAINER_ID
                    fi
                    echo "Desplegando nuevo contenedor..."
                    docker run -d -p 8006:8006 ${DOCKER_IMAGE}:${DOCKER_TAG}
                    '''
        ...
        stage('Apply Kubernetes Files') {
            steps {
                ...
                sh '''
                    if ! kubectl -n microservices-ci get service division-deployment --ignore-not-found > /dev/null 2>&1; then
                        echo "El servicio no existe, exponiéndolo ahora..."
                        kubectl -n microservices-ci expose deployment division-deployment --type=NodePort --port=8006
                    else
                        echo "El servicio division-deployment ya existe, no se necesita exponerlo nuevamente."
                    fi
                '''
                }
            }
